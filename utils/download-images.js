...
function readAllJson() {
  try {
    ...
    console.error('读取all.json文件失败:', error.message);
    return null;
  }
}
...
async function downloadAllImages() {
  ...
  function extractRawSrc(obj) {
    if (Array.isArray(obj)) {
      obj.forEach(item => extractRawSrc(item));
    } else if (typeof obj === 'object' && obj !== null) {
      if (obj.rawSrc) {
        imageUrls.push(obj.rawSrc);
      }
      Object.values(obj).forEach(value => extractRawSrc(value));
    }
  }
